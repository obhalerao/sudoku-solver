...
def bruteForce(pzl, neighborVals):
    ...
    for pos in poss:
        for sym in syms:
            newPzl = list(pzl)
            newPzl[pos] = sym
            newNeighborVals = [i.copy() for i in neighborVals]
            for i in NEIGHBORS[pos]:
                newNeighborVals[i].add(sym)
            bF = bruteForce(''.join(newPzl), newNeighborVals)
            if bF: return bF
    return ''
...
def main():
    ...
    for idx, puz in enumerate(puzzles):
        ...
        setGlobals(puz)
        start = default_timer()
        bF = bruteForce(puz, [set(puz[i] for i in NEIGHBORS[j])-set('.') for j in range(N*N)])
        end = default_timer()
